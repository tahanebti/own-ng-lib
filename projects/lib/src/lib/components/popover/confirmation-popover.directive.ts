import { ComponentFactory, ComponentFactoryResolver, ComponentRef, Directive, ElementRef, EventEmitter, HostListener, Injector, Input, OnChanges, OnDestroy, OnInit, Output, Renderer2, SimpleChanges, TemplateRef, ViewContainerRef } from '@angular/core';
...
@Directive({
  selector: '[tnConfirmationPopover]'
})
export class ConfirmationPopoverDirective implements OnDestroy, OnChanges, OnInit {
  ...
  private showPopover(): void {
    if (!this.popover && !this.isDisabled) {
      ...
      const componentFactory: ComponentFactory<PopoverComponent> = this.cfr.resolveComponentFactory(
        PopoverComponent
      );
